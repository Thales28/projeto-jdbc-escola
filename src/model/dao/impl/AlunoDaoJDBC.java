...
public class AlunoDaoJDBC implements Dao {
	...
	public void insert(Object obj) {
		...
		else if ((bolsista.getTipoBolsa().isBlank() || bolsista.getTipoBolsa() == "NÃO")
				&& bolsista.getValorMensalidade() == 0) {
		...
		try {
			ps = conn.prepareStatement(
					"INSERT INTO aluno " + "(cpf, nome, valorMensalidade, endereco, turma, creditos, bolsista) "
							+ "VALUES " + "(?, ?, ?, ?, ?, ?, ?) ");
	...
	public void updateCreditos(Bolsista bolsista, Double creditos) {
		...
		try {
			ps = conn.prepareStatement("UPDATE aluno " + "SET creditos = ? " + "WHERE cpf = ?");
	...
	public void update(Object obj) {
		...
		if (aluno == null) {
			...
			throw new DbException("Nome não preenchido");
		} else if (bolsista.getTurma() == null || bolsista.getTurma().getId() == 0
				|| bolsista.getTurma().getId() == null) {
		...
		if ((bolsista.getTipoBolsa().isBlank() || bolsista.getTipoBolsa() == "NÃO")
				&& bolsista.getValorMensalidade() == 0) {
			bolsista.setTipoBolsa("SIM");
		}
		...
		try {
			ps = conn.prepareStatement("UPDATE aluno "
					+ "SET nome = ?, valorMensalidade = ?, endereco = ?, turma = ?, bolsista = ? " + "WHERE cpf = ?");
